...
age_curve_data <- regdata %>%
  ...
  filter(year != 2025) %>%
  summarize(n_opererte = as.character(n_distinct(PID)),
...
gender_data <- regdata %>%
  ...
  mutate(total = sum(n)) %>%
  filter(f == TRUE) %>%
  mutate(pro_kvinne = as.character(round((n / total * 100), digits = 0))) %>%
  select(pro_kvinne)
...
key_data_table <- key_data %>%
  mutate(pro_kvinne = paste(pro_kvinne, "%", sep = ""),
...
key_data_table <- key_data_table %>%
  ...
  filter(Sykehus == "Nasjonalt") # lagt til denne filtreringen siden reg.leder kun ønsker visning av nasjonalt
