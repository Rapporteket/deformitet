...
figAndelerGrVar <- function(RegData = 0, hentData = 0, preprosess = 0,
  ...
  DeformVarSpes <- varTilrettelegg(
    RegData = RegData, valgtVar = valgtVar,
    figurtype = "andeler"
  )
  ...
  if (valgtVar == "liggetidPostOp") {
    KImaalGrenser <- c(0, 90, 100)
  }
  if (valgtVar == "fornoydBeh2aar") {
    KImaalGrenser <- c(0, 70, 90, 100)
  }
  if (valgtVar == "reOp") {
    KImaalGrenser <- c(0, 5, 100)
  }
  ...
  if (max(Ngr) < Ngrense) { # Dvs. hvis ALLE er mindre enn grensa.
    ...
    if (!is.na(KImaalGrenser[1])) {
      ...
      if (antMaalNivaa == 3) {
        maalOppTxt[2] <- "Moderat"
      }
      rect(
        xleft = KImaalGrenser[1:antMaalNivaa], xright = KImaalGrenser[2:(antMaalNivaa + 1)],
        ybottom = 0, ytop = max(pos), col = fargerMaalNiva[1:antMaalNivaa],
        density = tetth, angle = 60, border = NA
      )

      legend(
        x = 0.01 * xmax, y = posOver, yjust = 1,
        ncol = antMaalNivaa + 1,
        density = c(NA, tetth),
    ...
    legend(
      x = max(AndelerGrSort, na.rm = TRUE), y = posOver, xjust = 1, yjust = 0.2,
      cex = 0.95, lwd = 2, col = farger[2],
      legend = paste0("Hele landet", " (", sprintf("%.1f", AndelHele), "%), ", "N=", N),
      bty = "o", bg = "white", box.col = "white"
    )
    mtext(
      at = pos + max(pos) * 0.0045, GrNavnOgAnt,
      side = 2, las = 1, cex = cexShNavn, adj = 1, line = 0.25
    ) # Legge på navn som eget steg


    title(Tittel, line = 1, font.main = 1, cex.main = 1.3)

    text(
      x = AndelerGrSort + xmax * 0.01, y = pos + 0.1, andeltxt,
      las = 1, cex = 0.9, adj = 0, col = farger[1]
    ) # Andeler, hvert sykehus
