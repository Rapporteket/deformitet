        ...
        shiny::tabsetPanel(id = ns("tab"),
          ...
          shiny::tabPanel("Gjennomsnitt", value = "gjen",
            ...
            bslib::card_body(
              ...
              bslib::card_body(
                "Tabellen viser gjennomsnitt og median per sykehus og for hele landet.
    ...
    function(input, output, session){
      ...
      tabell <- shiny::reactive ({
        if(input$x_var %in% komplikasjon_typer) {
          # hvis "komplikasjonstype" er valgt, bruk kompl_reactive()
      ...
      figur <- shiny::reactive ({
        if(input$x_var %in% komplikasjon_typer) {
          ...
          deformitet::lag_ggplot_fordeling(
            tabell_reactive(),
            gg_data,
            my_data_reactive(),
            input$visning_type
      ...
      navn_reactive <- shiny::reactive({
        deformitet::mapping_navn(raw_data, input$x_var)
      })

      gjen_added_reactive <- shiny::reactive({

        if (input$x_var %in% c("Alder", "Knivtid", "Diff_prosent_kurve")) {
          deformitet::gjen_var_til_data(raw_data, data, input$x_var)
        } else {
          deformitet::gjen_var_til_data(raw_data, data, navn_reactive())
        }
      })
      ...
      gjen_data_reactive <- shiny::reactive({
        data.frame(gjen_prepVar_reactive()[1])
      })

      # Lag tabell:

      gjen_tabell_reactive <- shiny::reactive ({
        deformitet::lag_gjen_tabell(gjen_data_reactive())
      })
