...
preprosData <- function(RegData = RegData, egneVarNavn = 0) {
  ...
  if (egneVarNavn == 0) {
    RegData <- dplyr::rename(RegData,
      OpDato = .data$SURGERY_DATE,
      Kjonn = .data$GENDER,
      PasientID = .data$PATIENT_ID
    )
  }
  ...
  RegData <- dplyr::rename(RegData,
    ReshId = .data$CENTREID,
    ShNavn = .data$CENTRESHORTNAME
  )
...
prepVar <- function(
  ...
  if (visning == "over_tid") {
    ...
    my_data <- data |>
      dplyr::select(
        c("Sykehus", "CENTREID", dplyr::all_of({{ var }}), "Kjonn", "CURRENT_SURGERY")
      )
  }
