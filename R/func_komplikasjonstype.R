...
kompl_data <- function(regdata, var, var_kjønn, time1, time2, alder1, alder2, type_op, map_data){
  ...
  kompl <- kompl %>%
    dplyr::filter(
      Kjønn == dplyr::case_when(
        {{var_kjønn}} == "kvinne" ~ "kvinne",
      {{var_kjønn}} == "mann" ~ "mann",
  ...
  kompl <- kompl %>%
    dplyr::select(
      "PID", "Sykehus", "Kjønn",
      "CURRENT_SURGERY", "Blødning",
      "UVI", "Lunge", "DVT",
      "Emboli", "Inf_over", "Inf_dyp",
      "Inf_reop", "Lam", "Smerte", "Annet"
      )
  ...
  kompl <- kompl %>%
    dplyr::mutate(
      Komplikasjonstype = tidyr::replace_na(.data$Komplikasjonstype, "ukjent")
    ) %>%
    dplyr::filter(.data$Komplikasjonstype != "ukjent")
  ...
  kompl_df <- data.frame(
    table(
      kompl$Sykehus, kompl$Komplikasjonstype, kompl$Kjønn, kompl$CURRENT_SURGERY
    )
  )
  ...
  kompl_df <- dplyr::left_join(
    kompl_df,
    map_data,
    dplyr::join_by(.data$Sykehus == .data$orgname)
  )
