        ...
        shiny::mainPanel(
          bslib::navset_card_underline(
            bslib::nav_panel(
              ...
              shiny::h5("Antall registreringer pr. operasjonsdato"),
              paste0(
                "Denne tabellen gir en oversikt over antall registrerte operasjoner pr måned",
                "(i en gitt tidsperiode)."
              )
            ),
            bslib::nav_panel(
              "Antall registreringer pr. skjema",
              ...
              shiny::h5("Antall registreringer pr. skjema"),
              "Denne tabellen gir en oversikt over antall registrerte skjema (i en gitt tidsperiode)."
            )
          )
        )
      )
  ...
  shiny::moduleServer(
    ...
    function(input, output, session) {
        ...
        if (input$registreringer == "Egendefinert tidsperiode") {
          ...
          if (input$registreringer == "Siste år") {
            ...
            if (input$registreringer == "Siste måned") {
              date1 <- format(lubridate::ymd(Sys.Date() - months(1)), "%d-%m-%Y")
            } else {
              date1 <- format(lubridate::ymd(Sys.Date() - months(6)), "%d-%m-%Y")
            }
          }
        }
      })
      ...
      reg_reactive <- shiny::reactive({
        tbl_reg(date1(), date2(), data)
      })

      reg_skjema_reactive <- shiny::reactive({
        tbl_skjema_reg(date1(), date2(), data)
      })
