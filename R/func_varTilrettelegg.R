...
varTilrettelegg <- function(RegData, valgtVar, figurtype = "andeler") {
  ...
  if (valgtVar == "reOp") { # AndelGrVar,AndelTid,
    ...
    RegDataPas <- RegData_pas2op |>
      dplyr::group_by(.data$PasientID) |>
      dplyr::summarise(
        PasientID = .data$PasientID[1],
        Dager = sort(.data$OpDato[.data$CURRENT_SURGERY == 2])[1] - .data$OpDato[.data$CURRENT_SURGERY == 1]
      )
  ...
  if (valgtVar == "Alder") { # Fordeling, #AndelGrVar,AndelTid, GjsnGrVar, GjsnTid
    ...
    if (figurtype == "andeler") { # Fordelingsfigur
      ...
      # c(levels(RegData$VariabelGr)[-length(gr)], '90+')	#c(names(AndelLand)[-length(gr)], '90+')
  ...
  if (valgtVar == "Inngrep") {
    grtxt <- c(
      "Ikke klassifiserbar", "Fremre diskektomi, prolaps", "Bakre dekompresjon",
      "Fremre dekomp. SS u/prolaps", "Bakre fusjon", "Korporektomi", "Andre inngrep"
    ) # for verdiene 0:6
  ...
  if (valgtVar == "Komplinfek") { # AndelTid, #AndelGrVar
    ...
    # Kode 0,1: Nei, Ja +tomme
    ind <- intersect(
      which(RegData$OppFolgStatus3mnd == 1),
  ...
  if (valgtVar == "KomplStemme12mnd") { # AndelGrVar,
    ...
    # Kode 0,1: Nei, Ja +tomme
    RegData <- RegData[
      which(RegData$OppFolgStatus12mnd == 1) %i%
  ...
  if (valgtVar == "NRSsmerteArmEndr12mnd") { # AndelGrVar #GjsnGrVar
    ...
    if (figurtype %in% c("gjsnGrVar", "gjsnTid")) {
      RegData$Variabel <- RegData$NRSsmerteArmPreOp - RegData$NRSsmerteArm12mnd
    } # RegData$NRSEndr
  ...
  if (valgtVar %in% c("NytteOpr3mndAlleKat", "NytteOpr12mndAlleKat")) { # Andeler
    ...
    # 				'Verre enn noen gang', 'Ukjent')
    grtxt <- c(
      "Helt bra", "Mye bedre", "Litt bedre", "Uendret", "Litt verre", "Mye verre",
      "Verre enn noen gang", "Ukjent"
    )
  ...
  if (valgtVar == "Utdanning") { # Andeler  #AndelGrVar
    ...
    grtxt <- c(
      "Grunnskole, 7-10år", "Real-, yrkes- el vg skole",
