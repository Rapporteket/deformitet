    ...
    shiny::sidebarLayout(
      shiny::sidebarPanel(
        selectInput( # First select
          ...
          label = "Velg variabel:",
          choices = c(
            "Blodtap (ml)" = "PER_BLOOD_LOSS_VALUE",
        ...
        dateRangeInput( # second select
          inputId = ns("date"),
          label = "Tidsintervall:",
          start = Sys.Date() - 150, #start dato 150 dager før systemdato
          end = Sys.Date(), # end dato dagens dato
          min = "2023-01-01",
          max = Sys.Date(),
          format = "mm/dd/yy",
          separator = " - "
...
module_spc_server <- function (id) {
  shiny::moduleServer(
    ...
    function(input, output, session) {
      ...
      data_spc_reactive <- shiny::reactive({
        deformitet::prepros_SPC(data_reactive(), input$var, input$period)
      })

      #### MAKE SPC PLOT ##################################

      direction_reactive <- shiny::reactive({
      ...
      spc_plot_reactive <- shiny::reactive({
        deformitet::spc_function(
          data_spc_reactive(),
          value,
          tidsperiode,
          direction_reactive(),
      ...
      output$spc_plot <- shiny::renderPlot({
        spc_plot_reactive()
      })

      output$spc_table <- DT::renderDT({
        DT::datatable(
          data_spc_reactive(),
          extensions = 'Buttons',
          options = list(
